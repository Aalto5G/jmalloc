...
with open("allocs.txt", "r") as f:
  ...
  for line in content:
    ...
    if ptr not in ptrs:
      ptrs[ptr] = ptrcnt
      ptrcnt += 1
  if len(sys.argv) > 1 and sys.argv[1] not in ["ops"]:
  ...
  if len(sys.argv) > 1 and sys.argv[1] in ["ops"]:
    ...
    print("extern struct op ops[%d];" % (len(content),))
  if len(sys.argv) > 1 and sys.argv[1] not in ["ops"]:
    ...
    print("    {")
    if len(sys.argv) > 1 and sys.argv[1] in ["std", "stdset"]:
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["jmset", "stdset"]:
      print("      memset(ar[ops[i].loc], 0, ops[i].sz);")
